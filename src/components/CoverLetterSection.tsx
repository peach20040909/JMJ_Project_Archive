      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            프로젝트 경험과 트러블슈팅 이력을 바탕으로 자기소개서와 면접 대비 질문을 관리합니다.
          </p>
        </div>

        <button
          onClick={() => handleOpenNewModal()}
          className="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold flex items-center space-x-1.5 shadow-xs transition-colors self-start sm:self-auto"
        >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
          {QUESTION_TEMPLATES.map((tmpl, idx) => (
            <button
              ...
              className="p-2.5 rounded-lg bg-slate-50 hover:bg-indigo-50/70 border border-slate-200/60 hover:border-indigo-200 transition-colors text-left group space-y-1"
            >
              ...
              <div className="font-semibold text-slate-900 text-xs truncate group-hover:text-indigo-600">
                {tmpl.title}
              </div>
            </button>
          ))}
