...
export const TechStackMatrix: React.FC<TechStackMatrixProps> = ({
  ...
  return (
    <div className="space-y-6 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            프로그래밍 언어, 프레임워크, 개발 도구 역량과 숙련도를 관리합니다.
          </p>
        </div>

        <button
          onClick={handleOpenAdd}
          className="px-3 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-xs font-semibold flex items-center space-x-1 transition-colors shadow-xs self-start sm:self-auto"
        >
      ...
      {/* Filter Tabs */}
      <div className="flex items-center space-x-1 overflow-x-auto scrollbar-none bg-white p-2 rounded-xl border border-slate-200/80">
        {CATEGORIES.map(cat => (
          <button
            key={cat}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredSkills.map(skill => (
          <div
            key={skill.id}
            className="p-4 rounded-xl bg-white border border-slate-200/80 hover:border-slate-300 hover:shadow-xs transition-all flex flex-col justify-between space-y-3 group"
          >
